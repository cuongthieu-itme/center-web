...
export default function MyClasses() {
  const { getStudentClasses, studentClasses, studentClassesPagination, loading } = useStudentStore();
  const [currentPage, setCurrentPage] = useState(1);
  
  useEffect(() => {
    getStudentClasses(currentPage);
  }, [getStudentClasses, currentPage]);
  ...
  const handleNextPage = () => {
    if (currentPage < studentClassesPagination.last_page) {
      setCurrentPage(prevPage => prevPage + 1);
    }
  };
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      {studentClasses.length === 0 && !loading ? (
        ...
        <>
          <DataTable columns={columns} data={studentClasses} loading={loading} />
          
          {studentClassesPagination.last_page > 1 && (
            <div className="flex items-center justify-end space-x-2 py-4">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-medium">{studentClassesPagination.last_page}</span>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={handleNextPage}
                disabled={currentPage === studentClassesPagination.last_page}
              >
